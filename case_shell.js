...
var CaseShell = (function () {
  ...
  function renderTriage() {
    ...
    document.getElementById('tab-panel').innerHTML =
      ...
      '<div class="kv"><span>Classification</span><span style="font-weight:600;color:' + (c.classification==='DEFEND'?'var(--green)':c.classification==='ESCALATE'?'var(--red)':'var(--amber)') + '">' + escapeHtml(c.classification||'Pending') + '</span></div>' +
      ...
      (c.points&&c.points.length ? c.points.map(function(p){
        var col=p.evidenceStatus==='green'?'var(--green)':p.evidenceStatus==='amber'?'var(--amber)':'var(--red)';
        var icon=p.evidenceStatus==='green'?'ti-circle-check':p.evidenceStatus==='amber'?'ti-clock':'ti-circle-x';
        return '<div class="kv"><span style="display:flex;align-items:center;gap:6px"><i class="ti '+icon+'" style="color:'+col+'"></i>'+escapeHtml(p.claim)+'</span><span style="color:var(--text3);font-size:11px">'+escapeHtml(p.evidenceDoc||'')+'</span></div>';
  ...
  function renderEvidence() {
    ...
    document.getElementById('tab-panel').innerHTML =
      ...
      '<div style="flex:1;height:8px;border-radius:4px;background:var(--border)"><div style="width:'+pct+'%;height:100%;border-radius:4px;background:'+pctColor+'"></div></div>' +
      ...
      (c.points&&c.points.length ? c.points.map(function(p){
        var col=p.evidenceStatus==='green'?'var(--green)':p.evidenceStatus==='amber'?'var(--amber)':'var(--red)';
        var icon=p.evidenceStatus==='green'?'ti-circle-check':p.evidenceStatus==='amber'?'ti-clock':'ti-circle-x';
        return '<div class="kv"><span style="display:flex;align-items:center;gap:8px"><i class="ti '+icon+'" style="color:'+col+'"></i>'+escapeHtml(p.claim)+'</span><span style="color:var(--text3);font-size:11px">'+escapeHtml(p.evidenceDoc||'Outstanding')+'</span></div>';
  ...
  function renderDocuments() {
    ...
    document.getElementById('tab-panel').innerHTML =
      ...
      '<div class="kv"><span>Letter of Acknowledgement</span><span style="color:'+(c.loaStatus==='approved'?'var(--green)':'var(--text3)')+'">'+escapeHtml(c.loaStatus==='approved'?'✓ Sent':'○ Not sent')+'</span></div>' +
      ...
      '<p style="font-size:12px;color:var(--text2);margin-bottom:12px">AI-assisted drafting available when evidence pack is complete ('+(c.evidencePct||0)+'% done).</p>' +
      '<button class="btn-primary" '+(((c.evidencePct||0)<100)?'style="opacity:0.5;cursor:not-allowed" disabled':'')+' onclick="CaseShell.logActivity(\'Document generation initiated\',\'action\')"><i class="ti ti-file-pencil"></i> Generate Letter of Response</button>' +
      '</div>' +
      '</div>';
  }
